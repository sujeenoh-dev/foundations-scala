...
object PureFunctionsAnswer extends ZIOSpecDefault {
  def spec = 
    suite("Pure Functions") {
      ...
      suite("ZPure 기초") {
        ...
        test("flatMap") {
          ...
          val computation: ZPure[Nothing, Unit, Unit, Any, String, Int] = 
            ZPure.succeed(20).flatMap(a => divide(a, 5))
      ...
      suite("상태 관리") {
        ...
        test("modify state") {
          val computation: ZPure[Nothing, Int, Int, Any, Nothing, Int] = 
            ZPure.modify((s: Int) => (s * 2, s))  // (new state, return value)
      ...
      suite("환경과 의존성") {
        ...
        test("access environment") {
          ...
          val computation: ZPure[Nothing, Unit, Unit, Config, Nothing, Int] = 
            ZPure.access((config: Config) => config.multiplier * 5)
        ...
        test("state and environment") {
          ...
          val computation: ZPure[Nothing, Int, Int, Config, Nothing, Int] = 
            for {
              currentState <- ZPure.get[Int]
              config <- ZPure.access[Config](identity)
              _ <- ZPure.set(currentState + config.increment)
            } yield currentState
      ...
      suite("로깅과 출력") {
        ...
        test("logging") {
          val computation: ZPure[String, Unit, Unit, Any, Nothing, Int] = 
            ZPure.log("Computing result") *> ZPure.succeed(42)
      ...
      suite("에러 처리") {
        ...
        test("map errors") {
          ...
          val mapped: ZPure[Nothing, Unit, Unit, Any, Int, Int] = original.mapError(_.length)
      ...
      suite("Traverse 패턴") {
        ...
        test("stateful traverse") {
          ...
          val computation: ZPure[Nothing, Int, Int, Any, Nothing, List[Int]] = 
            ZPure.foreach(numbers)(addToTotal)
        ...
        test("traverse with errors") {
          ...
          val validComputation: ZPure[Nothing, Unit, Unit, Any, String, List[Int]] = 
            ZPure.foreach(validNumbers)(validatePositive)
          val invalidComputation: ZPure[Nothing, Unit, Unit, Any, String, List[Int]] = 
            ZPure.foreach(invalidNumbers)(validatePositive)
